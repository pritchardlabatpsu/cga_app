...
def getGrpCounts(isInSame_func, isInSame_sources_func, feat_summary, func_args=None):
    ...
    sameGs_src = [isInSame_sources_func(g,f,s,func_args) for g,f,s in zip(feat_summary.target,feat_summary.feat_genes,feat_summary.feat_sources)]
...
def generate_featSummary(varExp, outdir_sub = './'):
    ...
    for i in range(1,topN+1):
        feat_summary['feat_gene%d'%i]= feat_summary.apply(lambda x: x.feat_genes[i-1] if len(x.feat_genes)>(i-1) else '', axis=1) #get the nth most important feature, per gene
        feat_summary['feat_source%d'%i]= feat_summary.apply(lambda x: x.feat_sources[i-1] if len(x.feat_sources)>(i-1) else '', axis=1) #get the nth most important feature, per gene
...
def anlyz_varExp(varExp, suffix='', outdir_sub='./'):
    ...
    # Score ratios
    plt.figure()
    ax = sns.boxplot('feat_idx','varExp_ofFull', data=varExp.loc[(varExp.varExp_ofFull>0) & (np.abs(varExp.varExp_ofFull)!=np.inf),], color='royalblue')
    ...
    plt.figure()
    ax = sns.boxplot('feat_idx','varExp_ofRd', data=varExp.loc[(varExp.varExp_ofRd>0) & (np.abs(varExp.varExp_ofRd)!=np.inf),], color='royalblue')
    ...
    for n in range(1,max(varExp.feat_idx)+1):
        f.write('Median score (univariate, %s feature, non-neg score): %0.2f\n' % (int2ordinal(n),np.nanmedian(varExp.loc[varExp.score_ind>0,].loc[varExp.feat_idx==n,'score_ind'].astype(float))))
...
def anlyz_varExp_wSource(varExp, dm_data=None, suffix='', outdir_sub='./', ):
    ...
    feat_withceres = varExp.groupby('target').first().reset_index(drop=False).loc[:,['target','score_rd', 'score_full']]
...
def anlyz_varExp_feats(varExp, gs_dir = '../datasets/gene_sets/', outdir_sub='./'):
    ...
    sameGs_counts, sameGs_src_counts,feat_summary_annot = getGrpCounts(isInSameGS, isInSameGS_sources, feat_summary, genesets)
    plotGrpCounts(sameGs_counts, sameGs_src_counts, feat_summary_annot, 'in same gene set KEGG-Panther-Reactome', outdir_sub)
...
def anlyz_aggRes(aggRes, suffix='', outdir_sub='./'):
    ...
    # recall compares
    plt.figure()
    ax = sns.scatterplot(aggRes.recall_rd10, aggRes.sanger_recall_rd10, s=60, alpha=0.1, color='steelblue')
    ax.plot([0, 1.0], [0, 1.0], ls="--", c=".3")
    ax.set(xlabel='Recall P19Q3 test set (rd10 model)', ylabel='Recall Sanger (rd10 model)')
    plt.title('Recall; %s' % suffix)
    plt.savefig("%s/compr_recall_scatter_q3_q4.pdf" % (outdir_sub))
    plt.close()
    ...
    plt.figure()
    ax = sns.scatterplot(aggRes.sanger_score_rd10, aggRes.sanger_recall_rd10, s=60, alpha=0.1, color='steelblue')
    ax.set(xlabel='Score (rd10 model)', ylabel='Recall (rd10 model)', xlim=(0,1.1), ylim=(0,1.1))
    plt.title('Sanger; %s' % suffix)
    plt.savefig("%s/score_recall_Sanger.pdf" % (outdir_sub))
    plt.close()
