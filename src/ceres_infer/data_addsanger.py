...
class sanger_data:
    def __init__(self):
        self.dir_datasets = '../datasets/Depmap/Sanger'
        self.data_name = 'dataSanger'
        self.fname_gene_effect_sanger = 'gene_effect.csv'
        self.fname_gene_dependency_sanger = 'gene_dependency.csv'
        self.baseline_filter_stats = None

    def load_data(self, gene_dependency=False):
        self.gene_dependency = gene_dependency
        if gene_dependency:
            print('loading Sanger gene dependency...')
            self.df_crispr_sanger = pd.read_csv('%s/%s' % (self.dir_datasets,self.fname_gene_dependency_sanger), header=0)
        else:
            print('loading Sanger gene effect...')
            self.df_crispr_sanger = pd.read_csv('%s/%s' % (self.dir_datasets,self.fname_gene_effect_sanger), header=0)
    ...
    def preprocess_data(self):
        ...
        #crispr-Sanger
        print('processing Sanger CERES...')
        self.df_crispr_sanger = reindex(self.df_crispr_sanger)
        self.df_crispr_sanger.columns = self.df_crispr_sanger.columns + ' [CERES]'
        linesN_missing = max(self.df_crispr_sanger.isna().sum()) #maximum number of cell lines with missing data
        if linesN_missing>0:
            print('There are %d cell lines out of %d that will be dropped, due to missing data' %
                  (linesN_missing, self.df_crispr_sanger.shape[0]))
            self.df_crispr_sanger.dropna(axis=0, how='any',inplace=True) #remove cell lines with any NaNs

        if self.gene_dependency:
            #using Achilles gene dependency, convert proba to dependent/not dependent
            self.df_crispr_sanger = self.df_crispr_sanger > 0.5
            self.df_crispr_sanger = self.df_crispr_sanger.astype(int)
    ...
    def filter_baseline(self):
        ...
        self.df_crispr_sanger, c6 = removeInvariant(self.df_crispr_sanger)

        counts1Sanger = pd.DataFrame({'source':['ceres'],
                                'counts_invariant':[c6]})
        ...
        if self.gene_dependency:
            self.df_crispr_sanger, c6 = removeLowVariantCat(self.df_crispr_sanger)
        else:
            c6 = None

        counts2Sanger = pd.DataFrame({'source':['ceres'],
                               'counts_cat_lowvariant':[c6]})

        countsSanger = counts1Sanger.merge(counts2Sanger, on='source')
        ...
        if not self.gene_dependency:
            self.df_crispr_sanger, c6 = removeLowVariantCont(self.df_crispr_sanger)
        else:
            c6 = None

        counts3Sanger = pd.DataFrame({'source':['ceres'],
                               'counts_cont_lowvariant':[c6]})

        countsSanger = countsSanger.merge(counts3Sanger, on='source')

        #---
        #summary
        counts4Sanger = pd.DataFrame({'source':['ceres'],
                               'feats_left':[self.df_crispr_sanger.shape[1]]})

        countsSanger = countsSanger.merge(counts4Sanger, on='source')

        self.baseline_filter_stats_sanger = countsSanger


    def match_feats(self, dm_data):
        #match the dataset features to that in the provided dm_data
        self.df_crispr_sanger = self.df_crispr_sanger.loc[:,dm_data.df_crispr_sanger.columns]
    ...
    def printDataStats(self, outdir_sub='./'):
        ...
        f.write('CERES: %d cell lines and %d genes\n' % getDFinfo(self.df_crispr_sanger))
        f.close()

        #baseline filter stats
        if(self.baseline_filter_stats_sanger is not None):
            self.baseline_filter_stats_sanger.to_csv('%s/%s_base_filter_Sanger.csv' % (outdir_sub,self.data_name))

def build_data_gene_Sanger(datatype, dm_data, gene, sample_idx=None):
    ...
    df_crispr_shared_Sanger = dm_data.df_crispr_sanger

    #get the gene data
    df_crispr_y = df_crispr_shared_Sanger.filter(regex=('^%s\s' % gene))
    df_crispr_y_null = df_crispr_shared_Sanger.filter(regex=('^(?!%s\s)' % gene))
    df_crispr_rd = df_crispr_shared_Sanger.copy()
    ...
    if hasattr(dm_data, 'df_landmark') and (dm_data.df_landmark is not None):
        ...
        df_crispr_Lx_Sanger = df_crispr_shared_Sanger.loc[:,col_sel].copy()

    if df_crispr_y.shape[1] < 1:
        warnings.warn('gene %s not found in shared CERES dataset...skip' % gene)
...
def preprocessDataSanger(useGene_dependency, dir_out, dir_sanger = '../datasets/Depmap/'):
    ...
    # ------------------
    # parse Sanger data
    dm_data_Sanger = sanger_data()
    dm_data_Sanger.dir_datasets = os.path.join(dir_sanger, 'Sanger')
    dm_data_Sanger.data_name = 'dataSanger'
    dm_data_Sanger.load_data(useGene_dependency)
    dm_data_Sanger.preprocess_data()  # handles formatting and missing data
    dm_data_Sanger.filter_baseline()  # baseline filter, to remove invariant and low variant features
