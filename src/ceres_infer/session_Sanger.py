...
from ceres_infer.data_addsanger import depmap_data
from ceres_infer.data_addsanger import scale_data, build_data_gene,build_data_gene_Sanger, qc_feats, stats_Crispr
from ceres_infer.models_Sanger import depmap_model, selectUnivariate
from ceres_infer.models_Sanger import _featSelect_base as sf_base
from ceres_infer.utils import getFeatGene, getFeatSource
from ceres_infer.analyses_Sanger import *
...
class workflow:
    ...
    def load_processed_data(self):
        ...
        self.dm_data_Sanger = pickle.load(open(self.params['indir_dmdata_Sanger'], 'rb'))
    ...
    def _add_landmarks(self):
        ...
        self.dm_data_Sanger.df_landmark = pd.read_csv(self.params['indir_landmarks'], header=0)
    ...
    def infer(self):
        ...
        for gene2anlyz in tqdm(self.genes2analyz):
            ...
            # --- create datasets ---
            values = build_data_gene(self.params['model_data_source'], self.dm_data, gene2anlyz)
            if len(values) == 4:
            ...
            # Some genes are not found in the Sanger dataset
            valuessanger = build_data_gene_Sanger(self.params['model_data_source'], self.dm_data_Sanger, gene2anlyz)
            if len(valuessanger)==4:
                data_name, df_x_sanger, df_y_sanger, df_yn_sanger = valuessanger[0:4]
            elif len(valuessanger) == 3:
                continue
            # data_name, df_x_sanger, df_y_sanger, df_yn_sanger = build_data_gene_Sanger(self.params['model_data_source'], self.dm_data_Sanger, gene2anlyz)

            # data checks
            if (len(df_x) < 1 or len(df_y) < 1):  # empty x or y data
                continue

            df_x, df_x_sanger = qc_feats(df_x, df_x_sanger)
            ...
            x_sanger = df_x_sanger.values
            y_sanger = df_y_sanger.values.ravel()
            yn_sanger_vals = df_yn_sanger.values
            ...
            if (self.params['opt_scale_data']):
                ...
                if (any(to_scale_idx)):
                    x_train, x_test, x_sanger = scale_data(x_train, [x_test, x_sanger], to_scale_idx)
                else:
                    logging.info(
            ...
            x_tr, x_te, x_sanger_rd = sf_base().transform_set(x_train, x_test, x_sanger, feat_idx=feat_sel.index)

            # reduced model on the top N features
            data = {'train': {'x': x_tr, 'y': y_train},
                    'test': {'x': x_te, 'y': y_test},
                    'sanger': {'x': x_sanger_rd, 'y': y_sanger}}
            data_null = {'test': {'x': x_te, 'y': y_test, 'y_null': yn_test},
                         'sanger': {'x': x_sanger_rd, 'y': y_sanger, 'y_null': yn_sanger_vals}}
