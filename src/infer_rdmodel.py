...
session_notes = 'regression model, with random forest (iterative) and boruta feature selection; run on selective dependent genes (CERES std > 0.25 and CERES range > 0.6)'
...
pickle.dump(dm_data, open('%s/dm_data.pkl' % outdir_sub, 'wb'))
pickle.dump(dm_data_Q4, open('%s/dm_data_Q4.pkl' % outdir_sub, 'wb'))
